...
const Keyboard3D = dynamic(() => import("./Keyboard3D"), {
  ...
  loading: () => (
    <div className="w-full h-full flex items-center justify-center">
      <div className="w-12 h-12 border-4 border-subtle border-t-primary rounded-full animate-spin"></div>
    </div>
  ),
});
    ...
    >
      ...
      <div className="container mx-auto px-6 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        >
          ...
          <motion.h1
            className="text-3xl sm:text-4xl md:text-7xl font-bold text-foreground mb-6 !leading-[1.2]"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3 }}
          >
            {t.hero.title_prefix} <br />
            <span className="inline-block mt-3 text-transparent bg-clip-text bg-[linear-gradient(90deg,var(--shimmer-a)_0%,var(--shimmer-b)_40%,var(--shimmer-c)_50%,var(--shimmer-b)_60%,var(--shimmer-a)_100%)] bg-[length:200%_auto] animate-shimmer whitespace-nowrap">
              {t.hero.title_highlight}
            </span>
          </motion.h1>
          <motion.p
            className="text-foreground-muted text-lg md:text-xl mb-8 max-w-lg"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {t.hero.tags.map((tag, index) => (
              <div
                key={index}
                className="px-6 py-2 rounded-full border border-subtle text-foreground-secondary text-sm font-medium bg-surface/50 backdrop-blur-sm hover:border-primary hover:text-primary transition-colors cursor-default"
              >
                {tag}
              </div>
            ))}
